...
function getWrappedHeader<TData>(columns: ThermalPrinterTableColumnDef<TData>[]): string[][] {
  return columns.map((col) => wrapText(col.header ?? "", { width: col.width, wordBreak: "break-word" }))
}
...
function getWrappedRow<TData>(dataRow: TData, columns: ThermalPrinterTableColumnDef<TData>[]): string[][] {
  return map(columns, (col) => {
    ...
    const displayText = col.cell ? col.cell({ value: rawValue, row: dataRow }) : String(rawValue)
...
function getHeaderLines<TData>(columns: ThermalPrinterTableColumnDef<TData>[]): string[] {
