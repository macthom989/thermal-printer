...
const FONT_FAMILY = "Arial-Lgt, sans-serif"
...
export const printDoubleColumnBase64 = (
  ...
  const PADDING = 0
  const FONT_COLOR = "#cfcfcf"
  const lineHeight = fontSizePx * 1.5

  const descX = PADDING
  const strikethroughWidth = strikethroughText ? measureTextWidth(strikethroughText, fontSizePx) : 0

  const maxDescWidth = svgTotalWidth - PADDING - 10 - strikethroughWidth

  const descLines = description ? wrapText(description, maxDescWidth, fontSizePx, FONT_FAMILY) : []
  ...
  const descriptionSvgLines = descLines
    .map((line, index) => {
